...
export default function Creative({ user, state, updateState, activeDepartment }) {
  ...
  return (
    <div className="flex gap-0 h-[calc(100vh-6rem)] animate-fade-in">

      {/* ── Left sidebar: Personal Calendar ── */}
      <div className="w-80 lg:w-96 flex-shrink-0 border-r border-slate-800/60 bg-slate-950/30 overflow-y-auto hidden lg:block">
        <div className="p-3 border-b border-slate-800/60">
          <h3 className="text-xs font-bold text-slate-100 flex items-center gap-2">
            <CalendarIcon className="w-3.5 h-3.5 text-violet-400" /> My Calendar
          </h3>
        </div>
        <PersonalCalendar
          user={user}
      ...
      {/* ── Main content area ── */}
      <div className="flex-1 flex flex-col min-w-0 gap-3 p-3 overflow-hidden">

        {/* ── Header ── */}
        <div className="glass-card p-3 rounded-xl flex items-center justify-between gap-3 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-violet-500/10 rounded-lg text-violet-400">
              <DeptIcon className="w-4 h-4" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-slate-100">{activeDepartment}</h2>
              <p className="text-3xs text-slate-400">{deptTasks.length} task{deptTasks.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-0.5 bg-slate-950/60 p-0.5 rounded-lg border border-slate-800">
              {[
                ...
                <button key={f.val} onClick={() => setTimelineFilter(f.val)}
                  className={`px-1.5 py-0.5 rounded-md text-3xs font-medium transition ${
            ...
            {canAssignTasks && (
              <button onClick={() => setShowTaskForm(true)}
                className="p-1.5 rounded-lg bg-violet-600/20 hover:bg-violet-600/30 text-violet-400 transition" title="Add task">
        ...
        {/* ── Kanban columns ── */}
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2 min-h-0 overflow-hidden">
          {COLUMNS.map(col => {
            ...
            return (
              ...
              >
                <div className={`px-2.5 py-2 border-b border-slate-800/60 flex items-center justify-between ${style.bg} rounded-t-xl flex-shrink-0`}>
                  <div className="flex items-center gap-1.5">
                    <h3 className={`text-3xs font-bold uppercase tracking-wider ${style.header}`}>{col}</h3>
                ...
                <div className="flex-1 p-1.5 space-y-1 overflow-y-auto">
                  {colTasks.length === 0 ? (
                    <div className="flex items-center justify-center h-12 border border-dashed border-slate-800 rounded-lg mt-1">
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            className={`glass-card p-2 rounded-lg border-l-2 transition hover:border-l-violet-400 cursor-pointer ${
