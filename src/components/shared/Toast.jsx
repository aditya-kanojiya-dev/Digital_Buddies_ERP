...
function ToastItem({ toast, onRemove }) {
    ...
    return (
        <div
            className={`
        relative flex items-start gap-3 w-72 sm:w-80 p-3 sm:p-4 rounded-2xl
        border ${cfg.border}
        shadow-xl ${cfg.glow}
        animate-toast-in
        overflow-hidden
      `}
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                {toast.title && (
                    <p className="text-xs font-semibold text-slate-100 mb-0.5 truncate">{toast.title}</p>
                )}
            ...
            <button
                ...
                className="flex-shrink-0 text-slate-500 hover:text-slate-300 transition-colors mt-0.5 cursor-pointer p-1"
                aria-label="Dismiss"
            >
...
export function ToastProvider({ children }) {
    ...
    return (
        <ToastContext.Provider value={toast}>
            ...
            {toasts.length > 0 && (
                <div
                    className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-[var(--z-toast)] flex flex-col gap-2.5 max-w-[90vw] sm:max-w-none"
