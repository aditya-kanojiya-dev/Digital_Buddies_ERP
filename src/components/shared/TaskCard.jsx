            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                <div className="space-y-1.5 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5">
                        <span className={`text-xs px-2 py-0.5 rounded font-mono font-semibold ${statusBadgeColor}`}>
                            {task.status}
                        </span>
                        {task.priority && (
                    ...
                    >
                        <h4 className="font-bold text-sm sm:text-base text-slate-100 hover:text-violet-300 transition-colors">{task.title}</h4>
                    </button>
                    ...
                    <div className="flex flex-wrap gap-2 flex-shrink-0">
                        {task.status === 'New' || task.status === 'Assigned' ? (
                            <button
                                ...
                                className="bg-violet-600/20 hover:bg-violet-600/40 text-violet-400 px-3 py-1.5 rounded-lg border border-violet-500/25 transition-colors cursor-pointer font-bold text-xs"
                            >
                                Start Work
                            </button>
                        ) : null}
                        {task.status === 'In Progress' && (
                            <button
                                ...
                                className="bg-fuchsia-600/20 hover:bg-fuchsia-600/40 text-fuchsia-400 px-3 py-1.5 rounded-lg border border-fuchsia-500/25 transition-colors cursor-pointer font-bold text-xs"
                            >
                                Review
                            </button>
                        )}
                        {task.status === 'Review' && (
                            <button
                                ...
                                className="bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-400 px-3 py-1.5 rounded-lg border border-emerald-500/25 transition-colors cursor-pointer font-bold text-xs"
                            >
                                Done
                            </button>
                        )}
                        {!STATUS_PIPELINE.includes(task.status) && task.status !== 'Blocked' && (
                            <button
                                onClick={() => onStatusChange?.(task.id, 'In Progress')}
                                className="bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 px-3 py-1.5 rounded-lg border border-blue-500/25 transition-colors cursor-pointer font-bold text-xs"
                            >
                                In Progress
                            </button>
                        )}
                ...
                {viewMode === 'employee' && isCompleted && !renderActions && (
                    <span className="px-3 py-1.5 rounded-xl text-xs font-bold text-emerald-400 border border-emerald-500/20 flex-shrink-0">
                        ✔ Done
                    </span>
                )}
            </div>
                ...
                <div className="flex gap-2 pt-1">
                    {currentUser.id === task.assignedBy && (
                        <a
                            ...
                            className="flex-1 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-400 text-xs font-semibold py-2.5 rounded-lg border border-emerald-500/15 transition-colors flex items-center justify-center gap-1.5"
                        >
                            <Download className="w-4 h-4" /> Share Assets
                        </a>
                    )}
                    {(currentUser.id === task.assignedTo || currentUser.id === task.assignedTo2) && (
                        <a
                            ...
                            target="_blank" rel="noopener noreferrer"
                            className="flex-1 bg-green-600/10 hover:bg-green-600/20 text-green-400 text-xs font-semibold py-2.5 rounded-lg border border-green-500/15 transition-colors flex items-center justify-center gap-1.5"
                        >
                            <Send className="w-4 h-4" /> Request Assets
                        </a>
                    )}
                </div>
            )}
